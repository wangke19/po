...
func NewCmdAttachList(f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "attach-list <run-id>",
		Short: "List attachments for a test run",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			attachments, err := client.ListTestRunAttachments(cmd.Context(), args[0])
			if err != nil {
				return fmt.Errorf("list test run attachments %q: %w", args[0], err)
			}
